...
import { HealthInsurance as HealthInsuranceModel, Plan } from '../../interfaces/HealthInsurance';
...
const HealthInsurancePanel: React.FC = () => {
  ...
  const handleDeletePlan = async (id: number) => {
    ...
    try {
      ...
      // 2) Actualizar estado local â€” eliminar solo el plan con ese id
        ...
        setPlans((prev) =>
          prev.map((p) =>
            p.id === selectedPlan.id
              ? { ...p, name: savedPlan.name, healthInsuranceId: savedPlan.healthInsuranceId }
              : p
          )
        );
  ...
  // Obras Sociales
  const [insurances, setInsurances] = useState<HealthInsuranceModel[]>([]);
  ...
  const [selectedInsurance, setSelectedInsurance] = useState<HealthInsuranceModel | null>(null);
  ...
  const handleEditInsurance = (insurance: HealthInsuranceModel) => {
  ...
  const handleToggleActive = async (id: number, isActive: boolean) => {
    try {
      ...
      alert('âŒ Error al cambiar estado de la obra social.');
    }
  };
  ...
  const loadInsurancesAndPlans = async () => {
    ...
    try {
      ...
      toast.error('âŒ Error al cargar obras sociales y planes');
    } finally {
  ...
  return (
    <>
      <ConfirmModal
        ...
        onConfirm={async () => {
          if (planToDelete) {
            await handleDeletePlan(planToDelete.id);
          }
        }}
        onCancel={() => {
          setShowConfirm(false);
